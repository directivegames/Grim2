...
| Item | Notes |
| --- | --- |
| Exit Play Mode | While playing; `Esc` (games that handle Escape keep it) / `F8` (always exits) |
...
| Exit Play / Play Dev Console | While playing; Exit is `Esc` when the game does not use it, or `F8` always; Dev Console is F12 |
