...
4. World Exit — `node.destroy()` triggers `endPlay()`. `removeFromParent()` / `world.remove()` only detach the graph. Override `endPlay()`, call super, and run custom logic only when it returns true.
