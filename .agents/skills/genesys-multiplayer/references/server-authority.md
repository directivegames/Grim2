...
- Destroying networked nodes must also happen on the server. A client can request destruction via a `@ServerRPC`; the server calls `node.destroy()` / `MultiplayerSpawner.despawn()`.
