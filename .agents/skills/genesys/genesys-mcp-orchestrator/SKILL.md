...
The default surface is **compact**. Missing descriptors for hidden tools (`action_asset`, `action_prefab`, `query_asset`, `query_diagnostics`, navmesh tools) is expected. Missing descriptors for **deactivated** actor-era tools (`query_scene`, `query_actor`, `action_actor`, `action_component`) is also expected — prefer `query_node` / `action_node`.
...
- Prefer `query_node` / `action_node` for world/prefab scene-component trees.
...
4. Never use deactivated actor-era tools; prefer `query_node` / `action_node`.
