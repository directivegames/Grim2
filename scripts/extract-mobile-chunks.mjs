...
const groundTiles = [];
const spawnBlockers = [];
const innocentSpawnPoints = [];
let innocentProp = null;
for (const key of Object.keys(actors)) {
  ...
  // Spawn blockers: invisible static collision volumes that keep Grim in-bounds.
  // Recreated as real SpawnBlockerActors on mobile (they use default half-extents,
  // so only the transform matters).
  if (actor.$bc === 'GAME.SpawnBlockerActor') {
    spawnBlockers.push({
  ...
  // Innocent spawn markers: mission picks one per reveal (arrow points to it).
  // Recreated as real InnocentSpawnPointActors on mobile so innocents appear at
  // their authored places instead of random nav points.
  if (actor.$bc === 'GAME.InnocentSpawnPointActor') {
    innocentSpawnPoints.push({
      position: readVec(root.position, [0, 0, 0]),
      scale: readVec(root.scale, [1, 1, 1]),
      rotation: readVec(root.rotation, [0, 0, 0]),
    });
  ...
  // Innocent prop: InnocentHandler binds to a scene actor literally named "innocent"
  // and drives it at runtime. The mobile empty scene has none, so capture it here and
  // recreate it (hidden) so innocents can reveal. Name is forced to lowercase to match.
  if (nameLower === 'innocent') {
    innocentProp = {
      name: 'innocent',
      modelUrl,
...
function emitBlocker(b, index) {
  const lines = [
    `  {`,
    `    name: ${JSON.stringify(`MobileSpawnBlocker_${index}`)},`,
    `    position: ${vec3(b.position)},`,
  ];
  if (b.scale[0] !== 1 || b.scale[1] !== 1 || b.scale[2] !== 1) {
    lines.push(`    scale: ${vec3(b.scale)},`);
  }
  if (b.rotation[0] !== 0 || b.rotation[1] !== 0 || b.rotation[2] !== 0) {
    lines.push(`    rotation: ${euler(b.rotation)},`);
  }
  lines.push(`  },`);
  return lines.join('\n');
}

function emitSpawnPoint(p, index, namePrefix) {
  const lines = [
    `  {`,
    `    name: ${JSON.stringify(`${namePrefix}_${index}`)},`,
    `    position: ${vec3(p.position)},`,
  ];
  if (p.scale[0] !== 1 || p.scale[1] !== 1 || p.scale[2] !== 1) {
    lines.push(`    scale: ${vec3(p.scale)},`);
...
body += `export const SPAWN_BLOCKERS: readonly SpawnBlockerPlacement[] = [\n`;
body += spawnBlockers.map((b, i) => emitBlocker(b, i)).join('\n');
body += `\n];\n`;

body += `\n/** The single scene innocent prop (InnocentHandler binds to the actor named "innocent"). */\n`;
if (innocentProp) {
  body += `export const INNOCENT_PROP: GlbPlacement | null = ${emit(innocentProp).trim().replace(/,$/, '')};\n`;
} else {
  body += `export const INNOCENT_PROP: GlbPlacement | null = null;\n`;
}

body += `\n/** Innocent spawn markers — mission reveals innocents at these authored places. */\n`;
body += `export const INNOCENT_SPAWN_POINTS: readonly SpawnPointPlacement[] = [\n`;
body += innocentSpawnPoints.map((p, i) => emitSpawnPoint(p, i, 'MobileInnocentSpawnPoint')).join('\n');
body += `\n];\n`;
...
console.log(`Spawn blockers: ${spawnBlockers.length}`);
console.log(`Innocent prop: ${innocentProp ? `${innocentProp.modelUrl}` : 'NONE'}`);
console.log(`Innocent spawn points: ${innocentSpawnPoints.length}`);
