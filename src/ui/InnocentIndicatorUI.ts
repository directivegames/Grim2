...
const ON_SCREEN_HIDE_FRAC = 0.38;
...
export class InnocentIndicatorUI {
  ...
  public updateTarget(worldPos: THREE.Vector3 | null): void {
    ...
    const onScreen =
      this._targetScratch.z >= -1 &&
      this._targetScratch.z <= 1 &&
      sx >= w * ON_SCREEN_HIDE_FRAC &&
      sx <= w * (1 - ON_SCREEN_HIDE_FRAC) &&
      sy >= h * ON_SCREEN_HIDE_FRAC &&
      sy <= h * (1 - ON_SCREEN_HIDE_FRAC);
